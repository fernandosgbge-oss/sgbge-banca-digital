...
export function PWAInstallButton({ className }: { className?: string }) {
    ...
    return (
        <div className={`fixed z-50 ${className || 'top-6 left-0 right-0 px-4 flex justify-center'}`}>
            <div className="w-full max-w-sm relative">
                {showIOSHint && (
                    <div className="absolute top-full mt-4 w-full p-4 bg-white text-gray-800 text-sm rounded-xl shadow-2xl border border-gray-100 animate-fade-in-down">
                ...
                <button
                    onClick={handleClick}
                    className="group w-full flex items-center justify-between bg-white/95 backdrop-blur-md border border-sg-blue/20 text-sg-blue px-6 py-4 rounded-2xl shadow-2xl hover:shadow-sg-blue/30 hover:scale-[1.02] transition-all duration-300"
                >
